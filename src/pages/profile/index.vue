...
onShow(load)
async function load() { try { const profile = await appApi.getMe(); authStore.setProfile(profile); avatar.value = profile.avatar || ''; savedAvatar.value = avatar.value; previewFailed.value = false } catch (error) { showRequestError(error) } }
