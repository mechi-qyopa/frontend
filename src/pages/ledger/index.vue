<template>
  <view :class="['ledger-page', `theme-${themeStore.id}`]" :key="themeStore.id" :style="themeStore.pageStyle">
    <view class="range-bar" @click="openPicker">
      <view class="range-info"><image v-if="mascots" class="mini-cat" :src="mascots.mini" mode="aspectFit" /><view><text class="range-title">{{ periodTitle }}</text><text class="range-detail">{{ range.startDate }} 至 {{ range.endDate }}</text></view></view>
      <view class="range-actions"><view class="range-arrow" /></view>
    </view>
    <view class="summary-card"><view><text class="summary-label">收入</text><text class="income">+{{ formatAmount(summary.incomeTotal) }}</text></view><view><text class="summary-label">支出</text><text class="expense">-{{ formatAmount(summary.expenseTotal) }}</text></view><view><text class="summary-label">结余</text><text class="balance">{{ formatAmount(summary.netAmount) }}</text></view><image v-if="mascots" class="summary-avatar" :src="mascots.avatar" mode="aspectFit" /></view>
    <view class="list-title"><view class="list-title-main">流水明细 <image v-if="mascots" class="title-paw" :src="mascots.watermelon" mode="aspectFit" /> <text class="muted">{{ transactions.length }} 笔</text></view></view>
...
function getCategoryIcon(item) { const cat = resolveCategory(item, categoryMap.value); return categoryIcon(cat?.name, item.transactionType) }
...
.range-bar:active, .picker-tab:active, .nav-button:active, .date-cell:active, .week-cell:active { opacity: .7; }
...
.summary-card { gap: 12rpx; }
.summary-avatar { width: 88rpx; height: 88rpx; margin-left: 4rpx; padding: 8rpx; box-sizing: border-box; border-radius: 50%; background: rgba(255, 255, 255, .92); flex-shrink: 0; box-shadow: 0 4rpx 14rpx rgba(93, 58, 21, .18); }
.list-title-main { display: flex; align-items: center; }
.title-paw { width: 32rpx; height: 32rpx; margin-left: 10rpx; }
.empty { display: flex; flex-direction: column; align-items: center; gap: 6rpx; margin: 16rpx 24rpx 0; padding: 44rpx 30rpx; border-radius: var(--theme-radius-card); background: var(--theme-surface); color: var(--theme-text-muted); box-shadow: 0 8rpx 28rpx rgba(36, 58, 99, .04); }
.empty-cat { width: 170rpx; height: 170rpx; }
.ledger-cat-footer { display: flex; flex-direction: column; align-items: center; gap: 10rpx; margin: 16rpx 24rpx 0; padding: 34rpx 30rpx 30rpx; border-radius: var(--theme-radius-card); background: var(--theme-surface); box-shadow: 0 8rpx 28rpx rgba(36, 58, 99, .04); }
